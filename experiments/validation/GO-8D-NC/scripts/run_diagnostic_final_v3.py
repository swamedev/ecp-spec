...
inversions_count = 0
for w1, w2, w3 in set(weight_grid):
    dv3_w = w1 * conf_matrix + w2 * ged_ecp_matrix + w3 * ent_n12_matrix
    if not (np.all(dv3_w[:,0] > dv3_w[:,1]) and np.all(dv3_w[:,1] > dv3_w[:,2])):
        order_stable = False
...
opposite_AB = sum(1 for i in range(30) if len(set([np.sign(conf_matrix[i,1]-conf_matrix[i,0]), np.sign(ged_ecp_matrix[i,1]-ged_ecp_matrix[i,0]), np.sign(ent_n12_matrix[i,1]-ent_n12_matrix[i,0])])) > 1)
opposite_BC = sum(1 for i in range(30) if len(set([np.sign(conf_matrix[i,2]-conf_matrix[i,1]), np.sign(ged_ecp_matrix[i,2]-ged_ecp_matrix[i,1]), np.sign(ent_n12_matrix[i,2]-ent_n12_matrix[i,1])])) > 1)

score_H3 = 0
if 22 >= 5: score_H3 += 2
elif 22 >= 2: score_H3 += 1
if 20 >= 5: score_H3 += 2
elif 20 >= 2: score_H3 += 1
...
if not True:  # order_stable is False
    score_H3 += 2
...
if not (True and True):  # geo_AB and geo_BC both False
    score_H3 += 1

if not (True and True):  # harm_AB and harm_BC both False
    score_H3 += 1

print("Scores: H1=1, H2=1, H3=7")
