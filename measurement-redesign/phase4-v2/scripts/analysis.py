# -*- coding: utf-8 -*-
"""
MR-C7 v2 — analysis_v2.py (estatística, decisão e relatório; protocolo §8–§13).

Mesma lógica da v1 (analysis.py) — Wilcoxon/TOST, Holm, compliance, pisos, fidelidade,
vetos — com as adaptações do MR-C7-PROTOCOLO-PREREGISTRO-V2.md:
  - outputs MR-C7-REPORT-V2.{md,json} e MR-C7-CASE-REGISTRY-V2.yaml;
  - reporta os TRÊS componentes separadamente (conf, ged_ref, div_metric), §7.5;
  - reporta a penalidade residual de conf(V2) atribuível à cadeia `follows` (§7.4 v2:
    esperada ~3/caso; confirmar se dentro da banda δ_eq);
  - diagnóstico de causa §7.5.1 estendido: FAIL-tipo-A (conf), FAIL-tipo-B (diluição de
    agregação) e FAIL-tipo-C (defeito de construção do gerador), com verificação de
    sinal residual de tipo-C (overlap espúrio ΔV2 vs fatos-base não-pai).
"""
import os
import re
import json
...
PROTOCOLO_V2_HASH = "5a926b77c8baaf722a8eea1eccba43320ce00b676306f3d0042f4b7c1bb2627f"
...
def residual_tipo_c_check(states, cases_path):
    ...
    for case_id, c in sorted(cases.items()):
        ...
        # categoria pai = categoria intencional do candidato (candidate_id "<case>-V2-<cat>")
        for fact, cid in zip(v2_facts, v2_ids):
            n_v2_total += 1
            parent_cat = cid.split("-V2-")[1]
            # posição do pai em base_facts: fatos-base são gerados na ordem de base_cats
...
def follows_penalty_v2(recon_data):
    """Penalidade residual de conf(V2) atribuível à cadeia `follows` (§7.4 v2).
    ...
    arestas. Aqui medimos: (i) nº de arestas `follows` extras (reconstrução V2 - verdade
    V2); (ii) a penalidade em conf = conf_m(rec, verdade) - conf_m(rec, verdade+
    follows-extras).
    """
...
def main():
    ...
    # ---- penalidade residual conf(V2) via follows (§7.4 v2)
    ...
    # |conf(V2) - conf(V0)| dentro da banda? conf(V0) = 1.0 por construção (verdade V0 = G0 = rec V0)
    ...
    # ---- sinal residual de tipo-C (§7.5.1c)
    ...
    if not decision["PASS"]:
        ...
        if tipo_c["sinal_residual_tipo_c"]:
            ...
            cause = ("(b)", "FAIL-tipo-B — diluição de agregação",
                     conf_disc, comp_disc)
        else:
    ...
    report_json = {
        ...
        "nota_interpretativa": (
            "ged_ref tem limitação conhecida: a referência DATA-DRIVEN é construída só a "
            "partir de V0, então tende a não discriminar bem V1 de V3 — achado da v1, não "
            "corrigido nesta v2 (fora de escopo). Reportado por componente; não é sinal novo."
        ),
        "hashes": artifact_hashes,
    }

    with open(os.path.join(OUTPUTS, "MR-C7-REPORT-V2.json"), "w", encoding="utf-8") as f:
    ...
    with open(os.path.join(OUTPUTS, "MR-C7-CASE-REGISTRY-V2.yaml"), "w", encoding="utf-8") as f:
        yaml.safe_dump(case_registry, f, allow_unicode=True, sort_keys=False)

    # ---- hash dos artefatos de relatório
    report_json_hash = sha256_file(os.path.join(OUTPUTS, "MR-C7-REPORT-V2.json"))
    registry_hash = sha256_file(os.path.join(OUTPUTS, "MR-C7-CASE-REGISTRY-V2.yaml"))
    artifact_hashes["MR-C7-REPORT-V2.json"] = report_json_hash
    artifact_hashes["MR-C7-CASE-REGISTRY-V2.yaml"] = registry_hash

    # ---- MD report
    lines = []
    lines.append("# MR-C7-REPORT-V2")
    lines.append("")
    lines.append("## Gate MR-C7 (v2) — Validade Discriminante de Valor (DV-REDESIGN)")
    ...
    lines.append("|------|-------|")
    lines.append("| Pré-registro | `MR-C7-PROTOCOLO-PREREGISTRO-V2.md` (hash `%s`) |" % PROTOCOLO_V2_HASH)
    lines.append("| Correção do gerador | `C-GENERATOR-FIX-V2-PARAPHRASE.md` (hash `%s`) |" % GENERATOR_FIX_HASH)
    lines.append("| `generate_cases.py` (corrigido) | `%s` |" % GENERATOR_HASH)
    lines.append("| `cases.yaml` (v2, regenerado) | `%s` |" % CASES_V2_HASH)
    ...
    lines.append("")
    lines.append("| Estado | DV (média) | conf (média) | ged_ref (média) | div_metric (média) |")
    lines.append("|--------|------------|--------------|-----------------|--------------------|")
    for s in ("V0", "V1", "V2", "V3"):
        lines.append("| %s | %.6f | %.6f | %.6f | %.6f |" % (
            s,
            float(np.mean([dv[c][s] for c in case_ids])),
    ...
    lines.append("")
    lines.append("## 5.1 Penalidade residual de conf(V2) via cadeia `follows` (§7.4 v2)")
    lines.append("")
    lines.append("A verdade de V2 (G0 + self-loop do pai, excluída a cadeia `follows`) prevê "
                 "penalidade residual de conf(V2) restrita às arestas `follows` não-entailed "
                 "(~3/caso). Medido: média de arestas `follows` extras = **%.4f/caso**; "
                 "penalidade média em conf = **%.6f** (conf(V2) = %.6f; conf(V2) sem a "
                 "penalidade = %.6f). Δconf(V2)−V0 = **%.6f** (banda δ_eq = 0.05) → **%s**."
    ...
    lines.append("- **Não-circularidade (§6.4):** rótulos fixados pelo gerador+consenso antes de qualquer cálculo de DV; "
                 "decisão de §7.4 pré-registrada, irrevogável após inspeção de DV.")
    lines.append("")
    ...
    lines.append("")
    lines.append("Grafo de importação por script (módulos de fase4-v2 e externos):")
    ...
    lines.append("| Artefato | SHA-256 |")
    lines.append("|----------|---------|")
    lines.append("| `MR-C7-REPORT-V2.json` | `%s` |" % report_json_hash)
    lines.append("| `MR-C7-CASE-REGISTRY-V2.yaml` | `%s` |" % registry_hash)
    lines.append("| `MR-C7-REPORT-V2.md` | (hash deste relatório — registrado externamente após a escrita) |")
    lines.append("")
    ...
    if decision["PASS"]:
        lines.append("DV-REDESIGN autorizada para uso confirmatório **PROVISÓRIO** (limitado ao consenso "
                     ...
                     "Mesmo com PASS: validade PROVISÓRIA, sem decisão sobre GO-8E.")
    else:
    ...
    lines.append("")
    lines.append("**Assinatura:** Execução de gate MR-C7 v2 (consenso algorítmico, addendum 4ce9fa87...)")
    lines.append("**Data:** 2026-08-19")
    lines.append("")

    md_text = "\n".join(lines)
    md_path = os.path.join(OUTPUTS, "MR-C7-REPORT-V2.md")
    with open(md_path, "w", encoding="utf-8") as f:
        f.write(md_text)

    print("==== MR-C7-V2 SUMMARY ====")
