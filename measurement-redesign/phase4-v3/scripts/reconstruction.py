...
def build_v2_truth(base_facts, v2_facts):
    """Verdade de V2 (§7.4 v2 — MR-C7-PROTOCOLO-PREREGISTRO-V2.md).

    G0 + as arestas `relates` entailed pelos fatos ΔV2 (relação de cada entidade ΔV2
    com o fato-base pai, colapsando no self-loop do pai quando a categoria coincide).
    São EXCLUÍDAS a cadeia `follows` incidente em entidades ΔV2 e as `relates`
    ΔV2↔ΔV2 (não entailed; artefato mecânico de ordenação). Implementação determinística,
    hash-locked antes da execução (não-circularidade §6.4).
    """
    n_base = len(base_facts)
    units = [{"text": f, "af_id": "AF-%03d" % (i + 1)}
             for i, f in enumerate(base_facts + v2_facts)]
    entities, relations = build_entities_relations(units)
    v2_ids = set(e["entity_id"] for e in entities
                 if any(int(af.split("-")[1]) > n_base for af in e["source_af_ids"]))
    ent_cat = {e["entity_id"]: e["syn_category"] for e in entities}

    # G0 (apenas os fatos-base) — nós e arestas de referência.
    g0, _ = reconstruct_facts(base_facts)

    # Arestas `relates` entailed: ΔV2 ↔ base (relação com o pai). Colapsadas por categoria.
    extra_edges = []
    for r in relations:
        if r["relation_type"] != "relates":
            continue
        s_v2 = r["source"] in v2_ids
        t_v2 = r["target"] in v2_ids
        if s_v2 == t_v2:
            continue  # ΔV2↔ΔV2 (excluída) ou base↔base (já em G0)
        extra_edges.append((ent_cat[r["source"]], ent_cat[r["target"]], "relates"))

    edge_set = set((e["source"], e["target"], e["relation_type"]) for e in g0["edges"])
    for e in extra_edges:
        edge_set.add(e)

    node_set = set(n["syn_category"] for n in g0["nodes"])
    for s, t, _rt in extra_edges:
        node_set.add(s)
        node_set.add(t)

    return {
        "nodes": [{"node_id": c, "syn_category": c, "confidence": 1.0}
                  for c in sorted(node_set)],
        "edges": [{"source": s, "target": t, "relation_type": rt}
                  for (s, t, rt) in sorted(edge_set)],
    }


# ---------------------------------------------------------------------------
# Seeds (§7.5) — procedimento do generate_seeds.py congelado com seed_master 20260819.
# ---------------------------------------------------------------------------
SEED_MASTER = 20260819


def derive_seeds(n_cases):
    """PCG64(SeedSequence(20260819, spawn_key=(ci, 0))) -> 3 uint64 por caso."""
    out = {}
...
def main():
    ...
    for case_id in case_ids:
        ...
        # Verdade por estado (§7.4 v2): V1 = G0 estendido pelos fatos verdadeiros de ΔV1;
        # V2 = G0 + arestas `relates` ΔV2↔pai (self-loop; excluída a cadeia follows);
        # V3 = G0 (falsos não pertencem à verdade).
        truth_v1, _cats = reconstruct_facts(state_facts["V1"])
        truth_graphs[case_id]["V1"] = truth_v1
        truth_graphs[case_id]["V2"] = build_v2_truth(base_facts, c["states"]["V2"]["facts"])
