# MR-C7-PROTOCOLO-PREREGISTRO

## Gate MR-C7 — Validade Discriminante de Valor
## Protocolo de Pré-Registro (CONGELADO ANTES DE QUALQUER EXECUÇÃO)

**Documento:** MR-C7-PROTOCOLO-PREREGISTRO
**Fase:** Pré-registro (congelamento)
**Data:** 2026-08-18
**Status:** **CONGELADO — NÃO EXECUTADO.** Este documento congela o desenho, as estatísticas,
os limiares, os controles e as regras de decisão do gate **MR-C7** antes da execução. Qualquer
alteração posterior a este congelamento é **MUDANÇA DE ESPECIFICAÇÃO** e exige nova decisão de
governança (análogo a GOV-M-REDESIGN-01-GATE.md §7).
...
- Geração **procedural e determinística** por gerador sintético com semente fixa
  (`seed_casegen = 20260818`), implementado em script criado em `measurement-redesign/phase4/scripts/`
  e **hash-locked ANTES de rodar** (o conjunto de casos fica totalmente determinado pela semente
...
- **Namespace:** namespace procedural neutro e próprio do gate (categorias sintéticas
  ...
  implementado no script de fase 4 e hash-locked antes da execução.
...
- **Candidatos V2:** verdadeiros e **entailados** pelos fatos do caso-base (redundância por
  construção: paráfrases, subsumidos, instâncias já implicadas).
...
### 7.4 Grafos de verdade por estado (para conf)
| Estado | Grafo de verdade usado no F1 |
|--------|------------------------------|
| V0 | G0 (caso-base) |
| V1 | G0 estendido com os 3 fatos verdadeiros de ΔV1 |
| V2 | G0 (fatos de ΔV2 são entailados por G0 — sem mudança de verdade) |
...
#### 7.5.1 Distinção interpretativa de causa de FAIL (adendo aprovado pela governança — 2026-08-18)

Em caso de **MR-C7 = FAIL**, o relatório deve diagnosticar e declarar explicitamente qual das
duas causas abaixo (ou ambas) explica o resultado, **antes** de qualquer recomendação de
redesenho:

> **(a) FAIL por `conf`:** o componente `conf` isoladamente já não discrimina V1 de V3 acima do
> piso de efeito (§8.3) — indica problema real na medida de confiança de reconstrução.
>
> **(b) FAIL por diluição de agregação:** `conf` isoladamente discrimina V1 de V3 acima do piso
> de efeito, mas o composto (média 1:1:1 com `ged_ref` e/ou `div_metric`) cai abaixo do piso —
> indica que o **peso 1:1:1 está diluindo um sinal real**, não que os componentes individuais
> estão errados.
>
...
- Se a causa de FAIL for **controle quebrado** (§6.2) ou **violação de procedimento** (§10), o
  resultado é FAIL sem diagnóstico de métrica (não interpretável); novo desenho é requerido.
...
Execução em `measurement-redesign/phase4/`:
- `scripts/` — scripts de geração, rotulação, reconstrução e análise; **hash-lock ANTES de
  rodar** (SHA-256 registrado antes de qualquer execução).
- `inputs/` — casos-base, pools de candidatos, rótulos consensuais (registro completo).
- `outputs/`:
  - `MR-C7-REPORT.md` — relatório final em texto (padrão PHASE-B-REPORT.md) com **hash SHA-256**
    registrado;
  - `MR-C7-REPORT.json` — dados por caso, estatísticas por par, compliance, pisos, decisão, vetos;
  - `MR-C7-CASE-REGISTRY.yaml` — casos, estados, rótulos, seeds, consenso (padrão
...
decisão final (PASS/FAIL) e vetos. **Nenhum ajuste retroativo é permitido após a execução.**
...
| **MR-C7 FAIL** | Documentar como resultado válido. Recomendar **redesenho do componente de diversidade/estrutura** antes de nova tentativa. **Não** tentar "salvar" a métrica atual. GO-8E permanece NÃO AUTORIZADO |
...
> N=24, semente `20260818`), a construção exata de V0–V3 (K=3, pares estruturais casados,
> consenso de IA ≥2/3), a referência GED (DATA-DRIVEN por método, Fase B) e a agregação
> (aritmética 1:1:1, Fase B/C), a estatística de teste (P1–P5, Wilcoxon + TOST, Holm), os
> limiares (δ_eq = 0.05, α = 0.05, compliance ≥ 0.80, pisos de efeito, N efetivo ≥ 15), o
> tratamento de empates (§8), o tratamento de resultado negativo (§9), a regra de veto (§11) e
> as consequências da decisão (§14).
>
> **Nenhum arquivo travado por hash foi ou será alterado.** Todos os hashes da Seção de
> referências foram verificados antes deste congelamento.
>
> **Nenhuma execução ocorre até aprovação explícita da governança sobre este protocolo.**

---

## 16. Documentos de referência (hashes verificados em 2026-08-18)
...
**Data:** 2026-08-18
**Status:** PRÉ-REGISTRO CONGELADO — AGUARDANDO REVISÃO E APROVAÇÃO ANTES DA EXECUÇÃO